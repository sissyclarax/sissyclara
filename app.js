...
function render(){
  ...
  grid.innerHTML=shown.map(i=>`<article class="item-card ${i.ordered?'is-ordered':''}" data-id="${i.id}" tabindex="0"><div class="card-image">${imageMarkup(i)}<span class="category-pill">${i.ordered?t('ordered'):escapeHTML(t(i.section==='Clothes'?i.subcategory:i.section))}</span></div><div class="card-body"><h3>${escapeHTML(content(i.name))}</h3><p class="card-description">${i.description?escapeHTML(content(i.description)):t('noDescription')}</p><div class="card-tags">${i.tags.map(tag=>`<span class="tag">${escapeHTML(content(tag,'tag'))}</span>`).join('')}</div></div></article>`).join('');
  $('#emptyState').hidden=shown.length>0; $('#resultLabel').textContent=`${shown.length} ${shown.length===1?t('item'):t('items')}`;
}
function showDetail(id){const i=allItems.find(x=>x.id===id);if(!i)return;const location=i.section==='Clothes'?`${t('Clothes')} · ${t(i.subcategory)}`:t(i.section);const gallery=i.images.length>1?`<div class="detail-thumbnails">${i.images.map((src,index)=>`<button class="thumbnail ${index===0?'active':''}" data-gallery-image="${escapeHTML(src)}" aria-label="${t('viewImage')} ${index+1}">${imageMarkup(i,'',src)}</button>`).join('')}</div>`:'';const shopLink=i.url?`<a class="shop-link" href="${escapeHTML(i.url)}" target="_blank" rel="noopener noreferrer">${t('viewOnTemu')} <span>↗</span></a>`:'';$('#detailContent').innerHTML=`<div class="detail-layout"><div class="detail-media"><div class="detail-image-wrap">${imageMarkup(i,'detail-image')}</div>${gallery}</div><div class="detail-copy">${i.ordered?`<span class="order-status"><i></i> ${t('ordered')}</span>`:''}<p class="eyebrow">${escapeHTML(location)}</p><h2>${escapeHTML(content(i.name))}</h2><p class="description">${i.description?escapeHTML(content(i.description)):t('noDescription')}</p><div class="card-tags">${i.tags.map(tag=>`<span class="tag">${escapeHTML(content(tag,'tag'))}</span>`).join('')}</div>${shopLink}</div></div>`;$('#detailDialog').showModal()}
...
grid.addEventListener('click',e=>{const card=e.target.closest('.item-card');if(card)showDetail(card.dataset.id)});grid.addEventListener('keydown',e=>{if((e.key==='Enter'||e.key===' ')&&e.target.matches('.item-card'))showDetail(e.target.dataset.id)});
