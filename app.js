...
function render(){
  ...
  grid.innerHTML=shown.map(i=>`<article class="item-card ${i.ordered?'is-ordered':''}" data-id="${i.id}" tabindex="0"><div class="card-image">${imageMarkup(i)}<span class="category-pill">${i.ordered?t('ordered'):escapeHTML(t(i.section==='Clothes'?i.subcategory:i.section))}</span>${i.section==='Wishlist'?`<button class="like-button" data-like-key="${likeKey(i)}" aria-label="Like"><span>♡</span><b>0</b></button>`:''}</div><div class="card-body"><h3>${escapeHTML(content(i.name))}</h3><p class="card-description">${i.description?escapeHTML(content(i.description)):t('noDescription')}</p><div class="card-tags">${i.tags.map(tag=>`<span class="tag">${escapeHTML(content(tag,'tag'))}</span>`).join('')}</div></div></article>`).join('');
...
function showDetail(id){const i=allItems.find(x=>x.id===id);if(!i)return;const location=i.section==='Clothes'?`${t('Clothes')} · ${t(i.subcategory)}`:t(i.section);const gallery=i.images.length>1?`<div class="detail-thumbnails">${i.images.map((src,index)=>`<button class="thumbnail ${index===0?'active':''}" data-gallery-image="${escapeHTML(src)}" aria-label="${t('viewImage')} ${index+1}">${imageMarkup(i,'',src)}</button>`).join('')}</div>`:'';const shopLink=i.url?`<a class="shop-link" href="${escapeHTML(i.url)}" target="_blank" rel="noopener noreferrer">${t('viewOnTemu')} <span>↗</span></a>`:'';const likeButton=i.section==='Wishlist'?`<button class="like-button detail-like" data-like-key="${likeKey(i)}" aria-label="Like"><span>♡</span><b>0</b></button>`:'';$('#detailContent').innerHTML=`<div class="detail-layout"><div class="detail-media"><div class="detail-image-wrap">${imageMarkup(i,'detail-image')}</div>${gallery}</div><div class="detail-copy">${i.ordered?`<span class="order-status"><i></i> ${t('ordered')}</span>`:''}<p class="eyebrow">${escapeHTML(location)}</p><div class="detail-title-row"><h2>${escapeHTML(content(i.name))}</h2>${likeButton}</div><p class="description">${i.description?escapeHTML(content(i.description)):t('noDescription')}</p><div class="card-tags">${i.tags.map(tag=>`<span class="tag">${escapeHTML(content(tag,'tag'))}</span>`).join('')}</div>${shopLink}</div></div>`;$('#detailDialog').showModal();document.dispatchEvent(new CustomEvent('closet:rendered'))}
